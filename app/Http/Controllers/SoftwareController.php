...
class SoftwareController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('Softwares.software',['softwares'=>$softwares]);
    }
    /**
    ...
    {
        if(Auth::user() == null){
            return view('home'); //uzytkownik niezalogowany
        }
        $this->validate($request, ['title' => 'required|min:1|max:255',
    ...
    {
        $software_user_id = Software::find($id,['user_id']);
        if(Auth::user()->id != $software_user_id['user_id']){
            return back()->with(['success' => false, 'message_type' => 'danger']);
        }
        $this->validate($request, ['title' => 'required|min:1|max:255',
            'description' => 'required|min:10|max:500',
